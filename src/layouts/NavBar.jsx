...
function NavBar() {
  ...
  return (
    <nav className="w-full text-white absolute flex flex-col items-center bg-purple-600 justify-around">
          ...
          <div className="drawer sm:flex text-start min-w-[300px] sm:min-w-[410px] h-[100vh] flex-col sm:items-start gap-[147px] p-6 bg-gradient-to-b from-orange-600 to-orange-500 fixed top-0 left-0 shadow-2xl">
            <div className="flex h-[525px] flex-col items-center sm:items-start gap-8 self-stretch">
              ...
              <div className="lg:text-lg flex flex-col">
                ...
                {isLoggedIn() ? null : (<Anchor to={'/login'} className="p-3 text-lg hover:bg-white hover:text-orange-600 rounded-md w-[300px]">Log In</Anchor>)}
                ...
                {isLoggedIn() ? (<Anchor to={`/dashboard/${auth?.user?.role === 1 ? "admin" : "user"}`} className="p-3 text-lg hover:bg-white hover:text-orange-600 rounded-md w-[300px]">Admin Panel</Anchor>) : null}
