...
const SearchResultsPage = () => {
  ...
  let searchedProducts = useSelector((store) => store.products.searched_products)
  console.log(searchedProducts)
  const uniqueBrands = [...new Set(searchedProducts?.map((product) => product.brand))]; // Obtener marcas únicas
  const uniqueColors = [...new Set(searchedProducts?.map((product) => product.description[0]?.color))];

  const getProducts = async () => {
    dispatch(productsActions.searched_products());
  }
  ...
  content = <p className="text-center mt-6 content-center text-4xl py-6 w-[67.5vw] text-[#ff5757]">Product not found<spin><img className="w-[60vw]  py-9" src="/src/assets/images/logo2.png" alt="logo" /></spin></p>
  } else {
  ...
  return (
    ...
    <div className="flex items-center justify-center">
      <div className="flex flex-col mt-6 self-start w-[22.5vw] min-h-[36vh] ">
        <p className="font-bold justify-start text-2xl text-[rgb(42,51,66)] border-b border-[#ff5757] pb-4">Searched Products</p>
        <div className="my-4">
